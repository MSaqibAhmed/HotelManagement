...
const GuestCreateReservation = () => {
  ...
  // If somehow accessed by non-guest, just push back to dashboard home
  useEffect(() => {
    if (!isGuest) {
  ...
  const [formData, setFormData] = useState({
    roomType: ROOM_TYPES.includes(initialRoomType) ? initialRoomType : "",
    checkInDate: initialCheckIn,
    checkOutDate: initialCheckOut,
    adults: initialGuests > 0 ? Math.min(initialGuests, 10) : 1,
    children: 0,
  ...
  // Preview availability and pricing whenever key fields change
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate("/dashboard/guest/my-reservations");
    } catch (err) {
  ...
  const errorText = (field) =>
    errors[field] ? (
      <p className="text-red-500 text-xs font-semibold mt-2">{errors[field]}</p>
    ) : null;
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-5 sm:p-7 lg:p-8">
          <div className="mb-6 sm:mb-8 flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-gray-500 mt-1 text-sm sm:text-base">
                Select room type and dates. We will auto-select an available room and calculate the total.
              </p>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
                ...
                <div className="rounded-2xl border border-gray-200 bg-gray-50 p-4">
                  ...
                  <p className="text-2xl font-black text-[#1e266d] mt-1">
                    {previewLoading
                      ? "..."
                      : `Rs ${Number(preview.amount || 0).toLocaleString()}`}
                  </p>

                  <p className="text-xs text-gray-500 mt-1">
                    {preview.nights
                      ? `${preview.nights} night(s) • Base Rs ${Number(
                          preview.basePrice || 0
                        ).toLocaleString()}`
                  ...
                  {!!preview.capacity && (
                    <p className="text-xs text-gray-500 mt-1">
                      ...
                      {preview.extraPersons > 0
                        ? ` • Extra: ${preview.extraPersons} (+Rs ${Number(
            ...
            <p className="text-[11px] text-gray-500">
              You are booking as <span className="font-semibold">{user?.name || "guest"}</span>.
              Your reservation will appear in \"My Reservations\" once created.
            </p>
          </form>
        </div>
      </div>
...
export default GuestCreateReservation;
